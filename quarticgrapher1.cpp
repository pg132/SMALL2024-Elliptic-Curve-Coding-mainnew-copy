...
int main(int argc, char *argv[]) {
    ...
    for (int p : primes) {
        ...
        */
        ...
        for (int t = 0; t < p; ++t) {
            ...
            //RANK 6 STUFF
            /*
            A = (calculatePoly(t, p, allPowersOfT, {int(-1 * littleC), int(2 * littleB)}) * \
                (calculatePoly(t, p, allPowersOfT, {int(1 - bigA), 2, 1})) % p - \
                inverse(3, p) * calculatePoly(t, p, allPowersOfT, {int(-1 * bigB), int(2 * littleA)}) * \
                calculatePoly(t, p, allPowersOfT, {int(-1 * bigB), int(2 * littleA)}) % p) % p;
            A = (A+p) % p;

            B = 2 * (power_mod_p(inverse(3, p), 3, p) * power_mod_p(calculatePoly(t, p, allPowersOfT, {int(-1 * bigB), int(2 * littleA)}), 3, p) % p - \
                inverse(3, p) * calculatePoly(t, p, allPowersOfT, {(int)(-1 * bigB), (int)(2 * littleA)}) * calculatePoly(t, p, allPowersOfT, {int(-1 * bigC), int(2 * littleB)}) * \
                (calculatePoly(t, p, allPowersOfT, {(int)(1 - bigA), 2, 1})) % p + \
                calculatePoly(t, p, allPowersOfT, {(int)(-1 * bigD), (int)(2 * littleC)}) * power_mod_p(calculatePoly(t, p, allPowersOfT, {int(1 - bigA), 2, 1}), 2, p) % p) % p;
            B = (B+p) % p;
            */

            //cout << "A = " << A << ", B = " << B << endl;
            // Parity bias family with s = -12k^4 and k = 1
            /*int s = 12;
            int poly = (power_mod_p(t, 2, p) + s) % p;
            A = (((-1) * 3 * poly) % p + p) % p;
            B = (((-1) * 2 * t * poly) % p + p)%p;*/

            A = 1;
            // int num = 123;
            B = (power_mod_p(t, 3, p)+4*power_mod_p(t, 2, p)-power_mod_p(t, 1, p) /* + 7*power_mod_p(t, 2, p)*/) % p;
    ...
    std::ofstream output_file("data2.txt");
